...
namespace PlaneGame
{
	...
	public class AeroplaneController : MonoBehaviour
	{
		[SerializeField]
		private float
			m_MaxEnginePower = 40f;        // The maximum output of the engine.
		[SerializeField]
		private float
			m_Lift = 0.002f;               // The amount of lift generated by the aeroplane moving forwards.
		[SerializeField]
		private float
			m_ZeroLiftSpeed = 300;         // The speed at which lift is no longer applied.
		[SerializeField]
		private float
			m_StallSpeed = 24;
		[SerializeField]
		...
		[SerializeField]
		private float
			m_FlapsChangeSpeed = 0.1f;
		[SerializeField]
		private WheelCollider
			m_NoseWheel;
		[SerializeField]
		private List<WheelCollider>
			m_BrakedWheels; // List of Wheels to which brake force is applied.
		...
		public float WheelBrakes { get; private set; }
		...
		public float FlapsInput { get; private set; }
		...
		WheelCollider[] m_WheelColliders; // Necessary mostly for debug output
		...
		public void Move (float rollInput, float pitchInput, float yawInput, float throttleInput, float flapsInput, 
		                  bool airBrakes, float wheelBrakes)
		{
			...
			if (!grounded) {
				Debug.Log ((lineEnd - lineStart).magnitude);
			}
...
//			}
			...
			if (PitchInput == 0f) {
				// Always tend to pitch straight into the airstream
				// We're borrowing the pitch angle calculation from CalculateRollAndPitchAngles here.
				Vector3 actualDir = m_Rigidbody.velocity; // In contrast to the direction the plane is heading in!
				actualDir.Normalize ();
				Vector3 localActualDir = transform.InverseTransformDirection (actualDir);
				PitchInput = -Mathf.Atan2 (localActualDir.y, localActualDir.z) * m_AutoPitchLevel;
...
//			Throttle = Mathf.Clamp01 (Throttle + ThrottleInput * Time.deltaTime * m_ThrottleChangeSpeed);
		...
		private void CalculateAerodynamicEffect ()
		{
			...
			if (m_Rigidbody.velocity.magnitude > 0) {
				...
				// Simulate stalling by decreasing the aero factor below stall (zero lift) speed
				m_AeroFactor *= Mathf.Lerp (0, 1, Mathf.Clamp01 (ForwardSpeed / m_ZeroLiftSpeed));
				...
				// also rotate the plane towards the direction of movement - this should be a very small effect, but
				// means the plane ends up pointing downwards in a stall
...
//			var zeroLiftFactor = Mathf.InverseLerp (m_ZeroLiftSpeed, 0, ForwardSpeed);
			// Divisions are expensive, but I don't care at this point :)
			var dynamicStallSpeed = m_StallSpeed - Flaps * (m_StallSpeed / 2.0f);
			var stallFactor = Mathf.Clamp01 (Mathf.InverseLerp (dynamicStallSpeed - 10, dynamicStallSpeed, ForwardSpeed));
			// Calculate and add the lift power
			var liftPower = ForwardSpeed/* * ForwardSpeed*/ * m_Lift/* * zeroLiftFactor*/ * stallFactor * m_AeroFactor;
